...
class Oogoor(Module):
    def __init__(self, socket: int, robot_brain: rosys.hardware.RobotBrain, *, out_1: int = 0, out_1_p0: bool = False,
                 out_2: int = 0, out_2_p0: bool = False, out_3: int = 0, out_3_p0: bool = False, out_4: int = 0,
                 out_4_p0: bool = False) -> None:
        ...
        self.out_3 = out_3
        self.out_3_p0 = out_3_p0
        self.out_4 = out_4
        self.out_4_p0 = out_4_p0
        ...
        with ui.card():
            ...
            with ui.row():
                with ui.column():
                    ...
                    with ui.row():
                        ui.number('out_3 pin', format='%.f', value=0).bind_value(self, 'out_3')
                        ui.switch('p0', on_change=lambda e: ui.notify(
                            'Switched' if e.value else 'Unswitched')).bind_value(self, 'out_3_p0')
                        ui.button('send On', on_click=self.send_out_3)
                    with ui.row():
                        ui.number('out_4 pin', format='%.f', value=0).bind_value(self, 'out_4')
                        ui.switch('p0', on_change=lambda e: ui.notify(
                            'Switched' if e.value else 'Unswitched')).bind_value(self, 'out_4_p0')
